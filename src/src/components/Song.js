...
const Song = (props) => {
  const { id, upvotes, type, roomId, authToken, added } = props;
  ...
  const [voteStatus, setVoteStatus] = useState(
    localStorage.getItem(`upvoteCookie${id}`)
      ? localStorage.getItem(`upvoteCookie${id}`)
      : "notvoted"
  );
  ...
  const toggle = () => {
    ...
    setUpvoteCount(
      voteStatus === "notvoted" ? upvoteCount + 1 : upvoteCount - 1
    );
  ...
  return (
    <div
      class={"grid-song" + (!added ? " active" : "")}
      onClick={
        type !== "playlist" && canAdd && !added ? add : Function.prototype
      }
    >
      <div class="song-title">
        <img
          src={info ? info.image[0].url : "../logo.svg"}
          alt=""
          className="track-img"
        />
        &nbsp;&nbsp;&nbsp;&nbsp;
        {info && (
          <a
            className="link"
            href={info && type === "playlist" ? info.track_link : "/"}
          >
            {info.name}
          </a>
        )}
      ...
      <div class="song-artist">
        {info && (
          <a
            className="link"
            href={info && type === "playlist" ? info.artist_link : "/"}
          >
            {info.artists[0].name}
          </a>
        )}
      ...
      {type !== "playlist" && (
        <div className="song-votes">
          {added ? <i className="fa fa-check" /> : <i className="fa fa-plus" />}
        </div>
      )}
    </div>
  );
